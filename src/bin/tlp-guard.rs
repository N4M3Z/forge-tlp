...
fn main() -> ExitCode {
    ...
    let input = match serde_json::from_str::<HookInput>(&buf) {
        Ok(i) => i,
        Err(_) => return ExitCode::SUCCESS, // JSON parse error is Claude Code's responsibility
    };
    ...
    let classification = match tlp::classify_file(&file_path) {
        Some(c) => c,
        None => return ExitCode::SUCCESS, // File outside any vault — not our problem
    };
